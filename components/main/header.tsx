    ...
    <header className={clsx(
      ...
      <div className='layout'>
        <nav className='flex items-center justify-between py-4'>
          <ul className={clsx('flex items-center justify-between space-x-0.5',
          ...
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="flex bg-gray-200 dark:bg-gray-700 w-9 h-5 my-auto rounded-full focus:outline-none">
            <span className="text-white bg-gray-400 dark:bg-green-600 pl-0.5 pt-0.5 ml-0.5  w-4 h-4 my-auto rounded-full transition dark:dot ">
              {theme === 'dark' ? <FiSun className='' size={12} /> : <FiMoon size={12} className='text-white' />}
            </span>
          </button>
        </nav>  
      </div>
